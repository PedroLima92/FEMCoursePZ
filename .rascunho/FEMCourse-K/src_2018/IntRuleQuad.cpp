...
{
    if(order < 0) {
        std::cout << "IntRuleQuad:: SetOrder: NULL integration order\n";
        DebugStop();
    }
    SetOrder(order);
}
...
void IntRuleQuad::SetOrder(int order)
{
    ...
    if (fOrder==0||fOrder==1) {
        ...
        VecDouble ptsy(sqrt(npts),0.);
        this->gaulegQuad(-1.0, 1.0, ptsy, fWeights);
        for (int ipts= 0; ipts < sqrt(npts); ipts++) {
            fPoints(ipts,0) = ptsy[ipts];
        }
    }
    
    VecDouble co_i(3,0.);
    double weight_i = 0.;
    VecDouble wi = fWeights;
    VecDouble points_i = fPoints.GetCol(0);
    
    for (int ipts = 0; ipts < sqrt(npts); ipts++) {
        co_i[0] = points_i[ipts];
        weight_i = wi[ipts];
        
        for (int jpts = 0; jpts < sqrt(npts); jpts++) {
            int n = jpts + ipts*sqrt(npts);
            fPoints(n,0) = points_i[jpts];
            fPoints(n,1) = co_i[0];
            fWeights[n] = weight_i*wi[jpts];
        }
    }
}
